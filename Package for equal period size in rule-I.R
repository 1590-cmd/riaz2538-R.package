...
if((v-1)%%p==0)
{
  ...
  if(v%%p==0)
  {
    ...
    {
      i=(v-3)/p
      cat("MCPBRMD-II IS POSSIBLE IN EQUAL PERIOD SIZES FOR GIVEN", "V=",v,"i=",i,  "AND", "P=",p, "AND ALSO RUN D=4")
    }else
      if((v-2)%%p==0)
        {
        i=(v-2)/p
      cat("MCSPBRMD-II IS POSSIBLE IN EQUAL PERIOD SIZES FOR GIVEN", "V=",v,"i=",i,  "AND", "P=",p, "AND ALSO RUN D=8")}
}else
  if(v%%2==0){
    if((v-2)%%p==0){
      i=(v-2)/p
      cat("MCPBRMD-I IS POSSIBLE IN EQUAL PERIOD SIZES FOR GIVEN", "V=",v,"i=",i,  "AND", "P=",p, "AND ALSO RUN D=3")
    }else
      if(v%%p==0){
        i=v/p
     cat("MCWBRMD-I IS POSSIBLE IN EQUAL PERIOD SIZES FOR GIVEN", "V=",v,"i=",i,  "AND", "P=",p, "AND ALSO RUN D=5")
      }else
        if((v+1)%%p==0){
          i=(v+1)/p
          cat("MCSBGRMD-I IS POSSIBLE IN EQUAL PERIOD SIZES FOR GIVEN", "V=",v,"i=",i,  "AND", "P=",p, "AND ALSO RUN D=6")
        }else
          
          if((v-1)%%p==0){
            i=(v-1)/p
            cat("MCSPRMD-I IS POSSIBLE IN EQUAL PERIOD SIZES FOR GIVEN", "V=",v,"i=",i,  "AND", "P=",p, "AND ALSO RUN D=7")}
  }else
...
# example#1
(H<-CGSBRMD_equalsize(v=23,p=5,i=4,D=4))
H$G
(D<-design_CGSBRMD(H))
